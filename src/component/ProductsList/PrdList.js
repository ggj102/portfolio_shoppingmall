  ...
  for (let i = 0; i < num; i++) {
    pageNumArr.push(
      <a
        key={(i + 1).toString()}
        href="#pagenum"
        className={nowpage === i + 1 ? "page_focus" : "page_unfocus"}
        onClick={() => onpagefocus(i + 1)}
      >
        {i + 1}
      </a>
    );
  ...
  const sortMap = sortTypeArr.map((arr, idx) => {
    return (
      <li key={idx.toString()}>
        <a
          href="#sort"
          className={sortType === arr.type ? "sort_focus" : "sort_tab"}
          onClick={(e) => {
            e.preventDefault();
            onSort(arr.type);
          }}
          ...
          <div className="list_sort">
            ...
            <div className="sort_option">
              ...
              <div className="sort_view_type">
                <ul>
                  <li
                    className={listViewOption === 1 ? "option_tab_focus" : ""}
                    onClick={() => onlistViewOption(1)}
                  >
                    A
                  </li>
                  <li
                    className={listViewOption === 2 ? "option_tab_focus" : ""}
                    onClick={() => onlistViewOption(2)}
                  >
                    B
                  </li>
                  <li
                    className={listViewOption === 3 ? "option_tab_focus" : ""}
                    onClick={() => onlistViewOption(3)}
                  >
                    C
                  </li>
                  <li
                    className={listViewOption === 4 ? "option_tab_focus" : ""}
                    onClick={() => onlistViewOption(4)}
                  >
                    D
                  </li>
                </ul>
