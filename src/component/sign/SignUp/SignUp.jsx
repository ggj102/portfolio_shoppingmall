...
function SignUp({ history }) {
  ...
  const [reconfirmCheck, setReconfirmCheck] = useState({
    onClick: false,
    blank: false,
    wrong: false,
    form: false,
  ...
  const [birCheck, setBirCheck] = useState({
    onClick: false,
    vy: false,
    mm: false,
    dd: false,
  ...
  const necessaryInfo = () => {
    return <span className="Warning">필수 정보입니다.</span>;
  };
  ...
  const reconfirmWarning = () => {
    if (reconfirmCheck.wrong) {
      return <span className="Warning">비밀번호가 일치하지 않습니다.</span>;
  ...
  const onPwCheck = () => {
    ...
    if (stateCopy.value === "") {
      ...
      if (stateCopy.value.length <= 7 || stateCopy.value.length > 17) {
        ...
        !scText.test(stateCopy.value) ||
        !noNum.test(stateCopy.value) ||
        !noAZ.test(stateCopy.value) ||
        !noaz.test(stateCopy.value)
      ) {
        stateCopy.form = false;
        stateCopy.blank = true;
  ...
  const onBirCheck = () => {
    const noText =
      /[A-Z|a-z|ㄱ-ㅎ|ㅏ-ㅣ|가-힣|~!@#$%^&*()\-_=+|[\]'";,./<>?:{} ]/;
    const zero = "0";
    const stateCopy = { ...birCheck, onClick: true };

    if (
      stateCopy.vyValue.length < 4 || // 년도 길이가 4 미만일 경우
      parseInt(stateCopy.vyValue) < 1900 || // 혹은 년도가 1900 미만인 경우
      parseInt(stateCopy.vyValue) > year
    ) {
      // 현재 년도보다 높을 경우
      stateCopy.vy = false;
    } else if (stateCopy.vyValue.length === 4) {
      // 년도 길이가 4인 경우
      if (noText.test(stateCopy.vyValue)) {
        // 숫자가 아닌지 체크
        stateCopy.vy = false;
      }
      stateCopy.vy = true;
    ...
    if (
      stateCopy.ddValue === "" ||
      stateCopy.ddValue.length > 2 ||
      stateCopy.ddValue.indexOf(zero) === 0 ||
      parseInt(stateCopy.ddValue) > 31
    ) {
      stateCopy.dd = false;
    } else if (
      stateCopy.ddValue.length === 1 ||
      stateCopy.ddValue.length === 2
    ) {
      if (stateCopy.vy) {
        if (stateCopy.mm) {
      ...
      <div className="container">
        <div className="content">
          <div className="contentDetail">
            <div className="idPw">
              ...
              <div className="pwBox boxType">
                <input
                  type="password"
                  className="contentInput"
                  onChange={reconfirmChange}
                  onBlur={onReconfirmCheck}
                />
                {/* <span>자물쇠확인</span> */}
              </div>
            </div>
            ...
            <div className="email">
              <div className="contentTitleText">
                본인 확인 이메일
            ...
            <div className="joinBtnArea">
              <div className="joinBtn btnPrimary" onClick={onJoinBtn}>
                <span>가입하기</span>
              </div>
            </div>
          </div>
