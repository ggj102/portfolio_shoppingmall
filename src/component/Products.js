    ...
    if (prdImg.length > 1) {
      return (
        <span
          ...
          onMouseOver={() => onPaging(arr.url)}
        >
          <img src={arr.url} alt="img" />
        </span>
      );
    }
        ...
        <div className="list_count">
          {
            <button
              className="delBtn"
              onClick={() => onRemove(num, optionListArr, "option")}
            >
              X
            </button>
          }
          <span className="list_price">
            {arr.add_price
              ? arr.count * (arr.add_price + product_price)
              : arr.count * product_price}
          </span>
          <div className="list_input">
            <button
              onClick={() => {
                onCountBtn(optionListArr, num, "option", "minus");
              }}
            >
              -
            </button>
            <input
              value={arr.count}
              onChange={(e) => onChangeCount(optionListArr, e, num, "option")}
              onBlur={(e) => {
                onBlurCount(optionListArr, e, num, "option");
              }}
            />
            <button
              onClick={() => {
                onCountBtn(optionListArr, num, "option", "plus");
              }}
            >
              +
            </button>
    ...
    return (
      <li key={idx}>
        ...
        <div className="list_count">
          <button
            className="delBtn"
            onClick={() => onRemove(num, addPrdListArr, "addPrd")}
          >
            X
          </button>
          ...
          <div className="list_input">
            <button
              onClick={() => {
                onCountBtn(addPrdListArr, num, "addPrd", "minus");
              }}
            >
              -
            </button>
            <input
              value={arr.count}
              onChange={(e) => onChangeCount(addPrdListArr, e, num, "addPrd")}
            />
            <button
              onClick={() => {
                onCountBtn(addPrdListArr, num, "addPrd", "plus");
              }}
            >
              +
            </button>
        ...
        <div className=" _category_area h_area h_area_v2">
          <div className="loc">
            ...
            {categoryList.map((list, num, idx) => {
              return (
                <Fragment key={idx}>
                  <NavLink to={"/CategoryPrdList/" + list.id}>
                    {list.name}
                    {categoryList.length === num + 1 &&
                      "(총" + list.num + "개)"}{" "}
                  </NavLink>
                  {categoryList.length !== num + 1 && (
                    <span className="bar">{">"}</span>
          ...
          <div className="_show_area">
            <div className="prd_detail_basic">
              <div className="_image view">
                ...
                <div className="thumbnail_area thmb_lst more">{imgPaging}</div>
              </div>
              <div className="info">
                <div>
                  <div className="_copyable">
                    <p className="_easy_purchase_hide_area prd_num">
                      "상품번호 : "<span className="thm">{product_code}</span>
                    </p>
                        ...
                        <div className="area_cost">
                          <strong className="info_cost">
                            ...
                            {saleInfo && (
                              <>
                                <span className="dc">
                                  {saleInfo.discount_percent}
                                </span>
                                <span className="ori">
                                  {saleInfo.original_price}
                                  <span>원</span>
                                </span>
                              </>
                            )}
                    ...
                    {installmentState ? (
                      <Installment btn={offinstallment} />
                    ) : (
                      ""
                    )}
                          ...
                          <div className="cost">
                            <div className="delivery_price">
                              {(deliveryMethod.length &&
                                deliveryMethod[0].price) === 0
                                ? "무료"
                                : deliveryMethod.length &&
                                  deliveryMethod[0].price + "원"}
                            </div>
                            <div className="interval">
                              {deliveryMethod.length &&
                                deliveryMethod[0].additional_description}
                            </div>
                    ...
                    <div
                      className={
                        prdOption.length === 0 && addPrd.length === 0
                          ? ""
                          : "prd_option"
                      }
                    >
                      ...
                      {prdOption.length === 0 && (
                        <div className="prd_single">
                          <div className="list_input">
                            <button
                              onClick={() => {
                                onCountBtn(null, null, "single", "minus");
                              }}
                            >
                              -
                            </button>
                            <input
                              value={singleCount}
                              onChange={(e) =>
                                onChangeCount(null, e, null, "single")
                              }
                              onBlur={(e) => {
                                onBlurCount(null, e, null, "single");
                              }}
                            />
                            <button
                              onClick={() => {
                                onCountBtn(null, null, "single", "plus");
                              }}
                            >
                              +
                            </button>
